...
public class ResultRestController {
	...
	public DBResponse getResult(@PathVariable int uId, @PathVariable int subId)
	{
		...
		try {
			dbResponse = resultService.getResultByUserId(uId, subId);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/calculate/{uId}/{subId}", method = RequestMethod.GET,headers="Accept=application/json")
	public DBResponse calculateResult(@PathVariable int uId, @PathVariable int subId)
	{
		DBResponse dbResponse = new DBResponse();
		try {
			dbResponse = resultService.calculateResult(uId, subId);
		} catch (Exception e) {
			dbResponse.setOperationCode(OpCode.EXECPTION);
			dbResponse.setMessage(Message.SOMETHING_WENT_WRONG);
			e.printStackTrace();
