...
public class QuestionDAOImpl implements QuestionDAO{
	...
	@Override
	public int getTotalQuestion(int subId, Connection con) throws Exception {
		PreparedStatement st = null;
		int totalQuestion = 0;
		String q = "SELECT count(*) as count FROM  question_table where sub_id = ?";
		st = con.prepareStatement(q);
		st.setInt(1, subId);
		ResultSet rs = st.executeQuery();
		while (rs.next()) {
			totalQuestion = rs.getInt("count");
		}
		return totalQuestion;
	}

	@Override
	public List<QuestionVO> getAllQuestionBySubjectId(int subId, Connection con)
			throws Exception {
